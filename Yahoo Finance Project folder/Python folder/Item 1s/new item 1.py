#New item 1
#Logic:
#1) Take data from yahoo_fin module
#2) Data is taken from methods:
##get_balance_sheet()
##get_cash_flow()
##get_data() -> Historical data all the way from the beginning of time (not needed
##              but could be useful)
##get_holders()
##get_income_statement()    
##get_stats()

#3) Append all type of data in a single sheet.

#4) Do the same for all other stocks
...
##    combined_df.insert(0,'0', ["Symbol",stock])
##    #combined_df.insert(1,'1', ["Name",name])
##    combined_df.to_excel(writer, sheet_name="Sheet 1",startrow=0,index=False,header=False)
##    #startrow += 3
##               
##    writer.save()
